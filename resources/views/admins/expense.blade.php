...
<div class="Manage-expenses-heading d-flex justify-content-start rounded p-4" id="expense-heading">
    ...
    <div class="d-flex flex-column justify-content-center align-items-center align-content-center pb-2 pt-2 container">
        ...
        <form action="{{ route('expenses.store') }}" method="POST" class="d-flex flex-column gap-2 pb-3" id="expense-create-form">
            ...
            <input type="text" name= "payee" placeholder="Enter Name of Organisation/Individual" class="form-control" value={{ old('payee') }}>
            ...
            <select name="payment_modes_id"class="form-select payment-cursor">
                ...
                <option value="{{ $PaymentMode->id }}" {{ old('payment_modes_id') == $PaymentMode->id ? 'selected' : '' }}>
                    {{ $PaymentMode->name }}
                </option>
            ...
            <textarea name="comments"class="form-control"  value=>{{ old('comments') }}</textarea>
