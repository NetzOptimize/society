...
class AdminController extends Controller
{
    public function index($id=0)
    {
        ...
        if(isset($_GET['sort']))
        {
            $expenses= Expense::sort($_GET['sort'], $_GET['start_date'], $_GET['end_date'],$_GET['month']);
        ...
        elseif(isset($_GET['start_date']) && isset($_GET['end_date']))
        {
            ...
            if ( $startdate == true &&  $enddate == true) {
                ...
                return redirect()->route('expenses.index')->with('error','Invalid Request');
            }
        }
        elseif(isset($_GET['month']))
        {
            ...
            {
                $startdate = strtotime($_GET['month']);

                if ( $startdate == true) {
                $expenses = Expense::Monthlyfilter($_GET['month']);
                $count = $expenses->count();
                $sum = $expenses->sum('amount');
                }
                else {
        ...
        return view('admins.index', compact('expenses','sum', 'count','id','months'));
    }
