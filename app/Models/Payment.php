...

{
    ...
    public function scopeSort($query, $sort,$month,$start,$end)
    {
        ...
        if ($sort == 'Ascending') {
            ...
            if (!empty($month)) {
                if($month=='All') {
                    return $query->orderByRaw("STR_TO_DATE(dateofdeposit, '%d-%m-%Y') ASC")
                    ->get();
                }
                return $query->where('billingmonth', $month)
                    ->orderByRaw("STR_TO_DATE(dateofdeposit, '%d-%m-%Y') ASC")
                    ->get();
            }
            ...
            if (!empty($month)) {
                if($month=='All') {
                    return $query->orderByRaw("STR_TO_DATE(dateofdeposit, '%d-%m-%Y') DESC")
                    ->get();
                }
                return $query->where('billingmonth', $month)
                    ->orderByRaw("STR_TO_DATE(dateofdeposit, '%d-%m-%Y') DESC")
                    ->get();
            }
            if(!empty($start) && !empty($end))
            {
